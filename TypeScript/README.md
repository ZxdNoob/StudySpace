...
一个常量或变量声明的字面量如果**不显式注解类型**，就会是一个**字面量类型**（将字面量的值作为类型）。
...
* ReturnType<T>: TypeScript **内置的条件类型**。用于`函数类型` T **返回值**类型。
...
```typescript
  ...
  type ANumber = ReturnType<typeof add>; // add 函数的返回值是一个 number，所以 ANumber 是一个 number 类型
```
